...
df=read_tsv("./picrust_ex/pathways_out/path_abun_strat.tsv")
...
dfus=read_tsv("./picrust_ex/pathways_out/path_abun_unstrat.tsv")
...
dfk=read_tsv("./picrust_ex/KO_metagenome_out/pred_metagenome_unstrat.tsv")
...
OK, so before we go into the real-deal modeling, lets see real quick which if the kegg/pathways cluster by strain or facility. We get to see the null results in real time. 
...
Aldex2 provides a way to model compisitional (count) data, using a Welch test or Kruskal-Wallis for simple comparisons (two types), and a full blown glm for multiple variables at the same time (though I ran into issues getting the effect size and plotting). 
...
The aldex function wraps 3 steps, doing 
